(defun +lbs/paste-image ()
  ...
  (setq current-file-path     (or load-file-name buffer-file-name)
        ...
        target-image-default  (format-time-string "screenshot%Y%m%d%H%M%S.jpg" (current-time))
  ...
  (unless (file-name-extension target-image-name)
    (setq target-image-name (concat target-image-name ".jpg")))
  ...
  (call-process-shell-command (format "flameshot-to-file \"%s\"" target-image-path))
